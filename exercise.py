...
for i in range(len(my_dict['response']['results'])):
    my_dict['response']['results'][i]['views'] += 0
    print(my_dict['response']['results'])


def read_article(input_dict):
    random_article = random.randint(0, len(my_dict['response']['results']))
    input_dict['response']['results'][random_article]['views'] += 1
    return input_dict


my_dict = read_article(my_dict)
print(my_dict)


def display_views(input_dict):
    for i in range(len(my_dict['response']['results'])):
        temp_dict = my_dict['response']['results'][i]
        print(f'{temp_dict["webTitle"]}')
        print(f'read {temp_dict["views"]} times')
        print('')
